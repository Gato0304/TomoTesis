En este capítulo se explican los fundamentos necesarios para el modelado de la planta de segundo orden y el motor escogido para tener un referente en el sistema. Además, se realiza una comparación entre el circuito Sallen-Key y el sistema original.  

\section{Fundamentos}
Para el modelado del sistema se tomó en cuenta las ecuaciones de un motor CC contolado por armadura de segundo orden que cumple con las expresiones dadas en \ref{ec:motorarmadura}. De donde se obtiene el sistema de ecuaciones mostrado en \ref{ec:motorarmadura2}.

\begin{center}
	\begin{equation}
		\left\{
		\begin{matrix}
			&\dot{\omega }_{m}(t)=I_{a}(t)\left ( \frac{K_{i}}{J_{m}} \right )-\omega_{m}(t)\left (\frac{B_{m} }{J_{m}}\right )\\
			& \dot{I_{a}(t)} = e_{a}(t)\left( \frac{1}{L_{a}} \right)
			-I_{a}(t)\left ( \frac{R_{a}}{L_{a}} \right)
			-\omega_{m}(t)\left(   \frac{k_{m}}{L_{a}} \right)
		\end{matrix}
		\right.
		\label{ec:motorarmadura2}%
		%\textup{Ecuaciones de un motor DC controlado por armadura}	
	\end{equation}
\end{center}

%Se comenzó por emular un sistema de segundo orden  que permitiera obtener el comportamiento de la planta, para esto se tomó el sistema de un motor de segundo orden que cumple con las ecuaciones mostradas en \ref{ec:motorarmadura}.

Sabiendo que la corriente de campo es constante ($I_{f}$), se toman como variables de estado la velocidad angular del motor ($\omega_{m}$) y la corriente de armadura ($I_{a}$), obteniendo las ecuaciones \ref{ec:espaciodeestado1q} en el siguiente espacio de estado:
 
\begin{center}
	\begin{equation}
		\left\{\begin{matrix}
			\begin{bmatrix}
				& \dot{\omega _{m}(t)}\\ 
				& \dot{I_{a}(t)}
			\end{bmatrix}
			=
			\begin{bmatrix}
				& \frac{-B_{m}}{J_{m}} & \frac{K_{i}}{J_{m}} & \\ 
				&  \frac{-K_{m}}{L_{a}}& \frac{-R_{a}}{L_{a}} & \\ 
			\end{bmatrix}
			\begin{bmatrix}
				& \omega _{m}(t) \\ 
				& I_{a}(t)
			\end{bmatrix}
			+
			\begin{bmatrix}
				& 0 \\ 
				& \frac{1}{L_{a}}
			\end{bmatrix}
			e_{a}(t)\\ 
			y(t)=\begin{bmatrix}
				1 & 0
			\end{bmatrix}
			\begin{bmatrix}
				\omega_{m}(t)\\ 
				I_{a}(t)
			\end{bmatrix}
		\end{matrix}\right.	
		\label{ec:espaciodeestado1q}%
	\end{equation}
\end{center}

%Para el trabajo de grado se tomo en cuenta

Según \cite{kuo}  las constantes del par $k_{i}$ y la constante de fuerza contraelectromotriz $k_{b}$ son dos parámetros que están relacionados a través de la potencia mecánica, la cual se escribe de la siguiente manera:

\begin{center}
	\begin{equation}
		P=e_{b}(t)i_{a}(t)
		\label{ec:potencia}%
		%\textup{Ecuaciones de un motor DC controlado por armadura}	
	\end{equation}
\end{center}

También se puede expresar como:

\begin{center}
	\begin{equation}
		P= \tau_{m}(t)\omega_{m}(t)
		\label{ec:potencia2}%
		%\textup{Ecuaciones de un motor DC controlado por armadura}	
	\end{equation}
\end{center}

Se obtiene que:

\begin{center}

		
	$	k_{b} (V/rad/seg)= k_{i} (Nm/A)$
		\label{ec:potencia3}%
		%\textup{Ecuaciones de un motor DC controlado por armadura}	

\end{center}

Entonces en las unidades del SI, los valores de $k_{b}$ y $k_{i}$ son iguales.
 \section{DCMCT}
 
 
 Se estudió el sistema "Quanser DC Motor Control Trainer" (DCMCT), el cual será referenciado en este documento como motor Quanser. En las especificaciones técnicas de este dispositivo se exponen los parámetros indicados en la figura \ref{fig:Quanser}.
 
 
 
  \begin{figure}[H]
 	\centering
 	\includegraphics[width=1\linewidth]{img/quanser}
 	\caption{Parámetros Quanser}
 	\label{fig:Quanser}
 	\cite{quanser}
 \end{figure}  


Para la representación de los parámetros del motor Quanser en espacio de estados se toman los valores que están expuestos en la figura \ref{fig:Quanser}. 

Para el cálculo del momento de inercia equivalente del motor ($J_{equi}$) se tomó el parámetro $ J_{m}$ y los valores de la masa del disco $M_{l}=0.068kg$ y el radio del disco $r_{l}=0.0248m$.
 
 \begin{center}
 	\begin{equation}
 		J_{equi}=J_{m}+J_{l}\left ( \frac{r_{eje}}{r_{l}} \right )
 	\end{equation}
\end{center}

Donde $ J_{l}=\frac{M_{l}r_{l}^{2}}{2}$ y las relaciones entre los radios es de 1.



\begin{center}
	\begin{equation}
		J_{equi}=J_{m}+\frac{M_{l}r_{l}^{2}}{2}
	\end{equation}
\end{center}

A continuación en \ref{ec:valoresmotorarmadura} se agrupan los parámetros que se utilizarán para la representación del modelo del sistema.   
 \begin{center}
	 \begin{equation}
		\begin{matrix}
			K_{m}=0.0502\frac{Nm}{A}\\ 
			J_{equiv}= 21,4567\cdot10^{-6} Kg.m^{2}\\ 
			R_{a}=10.6\Omega\\ 
			L_{a}=0.82m H	
		\end{matrix}
		 \label{ec:valoresmotorarmadura}%
	\end{equation}
\end{center}

De acuerdo con la información presentada anteriormente, se obtiene el modelo de espacio de estado descrito con la ecuación \ref{ec:espaciodeestado1q} y \ref{ec:espaciodeestado2q}. Del cual se obtiene la función de transferencia indicada en la ecuación \ref{ec:plantaarmadura}. 


%El sistema de segundo orden emulará es el motor Quanser la cual cumple con las características anteriores, para así poder realizar las pruebas necesarias para evaluar el comportamiento de la planta con un controlador real. Al realizar el espacio de estado de la planta se obtiene%



\begin{center}
	\begin{equation}
		\left\{\begin{matrix}
			\begin{bmatrix}
				& \dot{\omega _{m}(t)}\\ 
				& \dot{I_{a}(t)}
			\end{bmatrix}
			=
			\begin{bmatrix}
				& 0 & 2340 & \\ 
				&  -61,22& -1,293\cdot10^{4} & \\ 
			\end{bmatrix}
			\begin{bmatrix}
				& \omega_{m}(t) \\ 
				& I_{a}(t)
			\end{bmatrix}
			+
			\begin{bmatrix}
				& 0 \\ 
				& 1220
			\end{bmatrix}
			e_{a}(t)\\ 
			Y=\begin{bmatrix}
				1 & 0
			\end{bmatrix}
			\begin{bmatrix}
				\omega_{m}(t)\\ 
				I_{a}(t)
			\end{bmatrix}
		\end{matrix}\right.
	\label{ec:espaciodeestado2q}%	
	\end{equation}
\end{center}

\begin{center}
	\begin{equation}
		\frac{W_{m}(s)}{E_{a}(s)}= \frac{2.8551\cdot10^{6}}{s^{2}+1.293\cdot10^{4}s+1.433\cdot10^{5}} 
		\label{ec:plantaarmadura}%
	\end{equation}
\end{center}

Según la ecuación \ref{ec:motorarmadura} $e_{m}=k_{m}\omega_{m}$ la ecuación \ref{ec:plantaarmadura} queda:

\begin{center}
	\begin{equation}
		\frac{E_{m}(s)}{E_{a}(s)}= \frac{1.433\cdot10^{5}}{s^{2}+1.293\cdot10^{4}s+1.433\cdot10^{5}} 
		\label{ec:plantaarmaduracompleta}%
	\end{equation}
\end{center}

Esto cumple con el modelo de un sistema de control segundo orden que corresponde a la ecuación \ref{ec:segundoorden}:

\begin{center}
	\begin{equation}\label{ec:segundoorden}
		F(s)=\frac{\omega_{n}^{2}}{s^{2}+2\zeta \omega_{n}s+\omega_{n}^{2}}
		%\textup{Ecuación que representa una planta de segundo orden}
	\end{equation}	
\end{center}

\section{Modelado del sistema con un circuito de segundo orden}

Con la funcion de transferencia planteada en  \ref{ec:filtroresistencia} y los valores obtenidos en \ref{ec:plantaarmaduracompleta}, se obtiene el siguiente sistema de ecuaciones:

\begin{center}
	\begin{equation}
		\left\{
		\begin{matrix}
			&   \omega_{n}^{2}=\frac{1}{R_{1}R_{2}C_{1}C_{2}} \\
			&   2\zeta \omega_{n}=\frac{R_{1}C_{1}+R_{2}C_{1}}{R_{1}R_{2}C_{1}C_{2}}
		\end{matrix}
		\right.
	\end{equation}
\end{center}

Donde los valores de $R_{1}, R_{2}, C_{1}, C_{2}$, $\omega_{n}$ y $\zeta$ son incógnitos. Primero se obtendrán los valores de  $\omega_{n}$ y $\zeta$ a partir de la función de transferencia \ref{ec:plantaarmaduracompleta} y del modelo de un sistema de control segundo orden planteado en \ref{ec:segundoorden} :




\begin{center}
	\begin{equation}
		\begin{matrix}
		 &	\omega_{n}^{2}=1.433\cdot10^{5} \\
		 &	\omega_{n}=378.549
		\end{matrix}	
	\end{equation}
\end{center}  

\begin{center}
	\begin{equation}
		\begin{matrix}
			&	2\zeta\omega_{n}=1.293\cdot10^{4} \\
			&	\zeta= 17.078
		\end{matrix}	
	\end{equation}
\end{center}

Al tener cuatro incógnitas y dos ecuaciones, se fijan los valores de $C_{1}= 1\mu F $ y $ C_{2}= 470nF  $, obteniendo: 

%$R_{1}=150k\Omega,  R_{2}=100k \Omega $

\begin{center}
	\begin{equation}
		\begin{matrix}
			& R_{1}R_{2}=\frac{1}{\omega_{n}^{2}C_{1}C_{2}} \\
			&R_{2}=\frac{1}{\omega_{n}^{2}C_{1}C_{2}R_{1}} 			
		\end{matrix}
	\end{equation}
\end{center}


\begin{center}
	\begin{equation}
			 2\zeta \omega_{n}=\frac{R_{1}C_{1}+R_{2}C_{1}}{R_{1}R_{2}C_{1}C_{2}}	
			 \label{ec:despeje}
	\end{equation}
\end{center}


Sustituyendo el valor de $R_{2}$ en \ref{ec:despeje} queda una ecuación cuadrática con incógnita $R_{1}$ 

\begin{center}
	\begin{equation}
		R_{1}^{2}-\frac{2\zeta}{\omega_{n}C_{1}}  R_{1}+ \frac{1}{\omega_{n}^{2}C_{1}C_{2}}	= 0
		\label{ec:despejefinal}
	\end{equation}
\end{center}

Los valores que arroja la ecuación son $R_{1}$= $\left\lbrace   90128; 164.86\right\rbrace \Omega$, tomando el valor de $R_{1}$=$90128\Omega$ el valor de $R_{2}= 164.86 \Omega$.

 La solución según el estándar E24 sería: $R_{1}=91k\Omega$ y $  R_{2}=160 \Omega $.

A continuación, se comparan las gráficas en la plataforma Matlab del sistema sin controlar y el modelo con el filtro Sallen-Key tambien emulado en Matlab pero con los valores de las resistencias y los condensadores.




\begin{center}
	\begin{figure}[H]
	\rotatebox[origin=c]{-90}{	\includegraphics[scale=0.50]{img/comparacion.eps}}
	
	\caption{Comparación entre el sistema original y Sallen-Key}
	\label{fig:QuanservsSallen}
\end{figure}
\end{center}
	
Se observa en el gráfico los siguientes resultados:

\begin{table}[htbp]
	\centering
	\caption{Parámetros de la comparación del sistema original y el circuito Sallen-Key en Matlab.}
	\resizebox{15cm}{!} {
	\begin{tabular}{|l|l|l|}
		\hline
		\textbf{Sistemas considerados} & \textbf{tiempo de alza$(t_{r})$ (seg)} & \textbf{tiempo de establecimiento $(t_{s})$ (seg)} \\
		\hline
		Sallen-Key & 0.198 s & 0.353 s \\
		\hline
		Sistema original & 0.2 s & 0.356 s \\
		\hline
	\end{tabular}%
	\label{tab:sistemas}%
}
\end{table}%

Con la tabla \ref{tab:sistemas} se estudia el error porcentual asociado a estos modelos con la ecuación  \ref{ec:errorporcentual}: 
\begin{center}
	\begin{equation}
		Error= \left |\frac{Valor_{sistemaoriginal} - Valor_{Sallen-Key}}{Valor_{sistemaoriginal}}*100 \%\right |	
		\label{ec:errorporcentual}
	\end{equation}
\end{center}

De allí se obtiene que para el tiempo de alza ($ t_{r}$) y para el tiempo de establecimiento ($t_{s}$) los errores serían:
%Se observa que el error porcentual calculado con \ref{ec:errorporcentual} dan los siguientes valores para el tiempo de alza y el tiempo de establecimiento:

\begin{center}
	\begin{equation}
		Error t_{r}= \left |\frac{0.198 - 0.2}{0.198}*100\%\right | =1.01\%	
		\label{ec:errorporcentualtalza}
	\end{equation}
\end{center}



\begin{center}
	\begin{equation}
		Error t_{s}= \left |\frac{0.353 - 0.356}{0.353}*100\%\right | =0.849\%	
		\label{ec:errorporcentualtesta}
	\end{equation}
\end{center}
%Los errores al ser menores al 2$\%$ son valores validos para la aproximación de nuestro modelo.

Al ser diferencias menores a dos cifras significativas o inferiores al 2$\%$ del valor teórico, se considera que es una aproximación válida para modelar el sistema.
%
%Para una planta de segundo orden genérica se tiene la ecuación \ref{ec:plantasegundo}
%\begin{center}
%	\begin{equation}
%		\frac{Y(s)}{U(s)}= \frac{b_{2}s^{2}+b_{1}s+b_{0}}{a_{2}s^{2}+a_{1}s+a_{0}} 
%		\label{ec:plantasegundo}%
%	\end{equation}
%\end{center}
%
%Como se pudo observar los motores de segundo orden que se van a estudiar tienen los valores de $b_{1}=0$ y $b_{2}=0 $ entonces la ecuación quedaría 
%
%\begin{center}
%	\begin{equation}
%		\frac{Y(s)}{U(s)}= \frac{b_{0}}{a_{2}s^{2}+a_{1}s+a_{0}} 
%		\label{ec:plantasegundo}%
%	\end{equation}
%\end{center}
%
%Resolviendo las ecuaciones diferenciales se tiene que 
%
%\begin{center}
%	\begin{equation}
%		\begin{matrix}
%		a_{2}\ddot{y}+a_{1}\dot{y}+a_{0}y=b_{0}U \\
%	 	x_{1}=y\\
%		\dot{x_{1}}=\dot{y}=\dot{x_{2}}\\
%		\ddot{x_{1}}=\ddot{y}=x_{3}\\
%		\label{ec:solecdiferencial2}%
%		\end{matrix}
%	\end{equation}
%\end{center}
%
%Obteniendo el siguiente espacio de estado
%
%\begin{center}
%	\begin{equation}
%		\begin{bmatrix}
%			\dot{x_{1}}\\
%			\dot{x_{2}}
%		\end{bmatrix}
%		=
%		\begin{bmatrix}
%			0 & 1\\ 
%			-\frac{a_{0}}{a_{2}}& -\frac{a_{1}}{a_{2}}
%		\end{bmatrix}x+
%		\begin{bmatrix}
%			0\\ 
%			\frac{b_{0}}{a_{2}}
%		\end{bmatrix}
%		\label{ec:espaciodestado2}%
%	\end{equation}
%\end{center}
\section{Modelo discreto de la planta de segundo orden}\label{sec:discplan}

\subsection{Diagrama de Bode}
Para la discretización del modelo de la planta indicado por las ecuaciones \ref{ec:espaciodeestado1q} y \ref{ec:espaciodeestado2q}, se debe analizar la respuesta en frecuencia del sistema continuo.

Entonces, partiendo de la ecuación \ref{ec:espaciodeestado2q}:
\begin{center}
	\begin{equation}
		\left\{\begin{matrix}
			\begin{bmatrix}
				& \dot{\omega _{m}(t)}\\ 
				& \dot{I_{a}(t)}
			\end{bmatrix}
			=
			\begin{bmatrix}
				& 0 & 2340 & \\ 
				&  -61,22& -1,293\cdot10^{4} & \\ 
			\end{bmatrix}
			\begin{bmatrix}
				& \omega_{m}(t) \\ 
				& I_{a}(t)
			\end{bmatrix}
			+
			\begin{bmatrix}
				& 0 \\ 
				& 1220
			\end{bmatrix}
			e_{a}(t)\\ 
			Y=\begin{bmatrix}
				1 & 0
			\end{bmatrix}
			\begin{bmatrix}
				\omega_{m}(t)\\ 
				I_{a}(t)
			\end{bmatrix}
		\end{matrix}\right.
	%	\label{ec:espaciodeestado2q}%	
	\end{equation}
\end{center}

Se procede hacer el análisis del sistema realimentado unitariamente, como se indica en la figura \ref{fig:bloquesistemarealimentado}.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{img/sistemarealimentado.jpg}
	\caption{Diagrama del sistema continuo realimentado.}
	\label{fig:bloquesistemarealimentado}
\end{figure}


 Luego, se realiza el diagrama de bode de magnitud de acuerdo con la ecuación  \ref{ec:bodemagnitud} y a través del programa \ref{code1} en MATLAB.

\begin{center}
		\begin{equation}
	{|F(\omega)|_{dB}}= 20\log {|F(\omega)|} 
	\label{ec:bodemagnitud}%
\end{equation}
\end{center}

\begin{lstlisting}[caption = {Sistema de espacio de estados en Matlab},captionpos=b,label={code1},language=Matlab]
	% Constantes
	km=0.0502;
	jm=21.456E-6;
	rm=10.6;
	lm=0.82E-3;
	
	% Espacio de estados
	A=[0 km/jm; -km/lm -rm/lm];
	B=[0;1/lm];
	C=[1 0];
	D=0;
	
	sistemacont=ss(A,B,C,D);
	
	% Diagrama de Bode
	bode(feedback(sistemacont,1))
	
	%Ancho de banda
	bandwidth(feedback(sistemacont,1))
\end{lstlisting}


De donde se obtiene el diagrama de la figura \ref{fig:bodesistemamagnitud}.
	\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{img/bodesistemamagnitud.png}
	\caption{Respuesta del diagrama de Bode magnitud.}
	\label{fig:bodesistemamagnitud}
\end{figure}

Y el ancho de banda es de $\omega_{B}=  235.4757 rad/s$.


\subsection{Tiempo de muestreo}

De acuerdo al teorema de Nyquist-Shannon \cite{Shannon} la frecuencia de muestreo $f_{s}$ debe ser mayor del doble de la frecuencia de Nyquist $f_{N}$. Siendo $f_{N}$ la frecuencia más alta medida en la señal. Tal como se indica en la ecuación \ref{ec:teoremaNyS}

	\begin{equation}
		{f_{s}}> 2 {f_{N}} 
		\label{ec:teoremaNyS}%
	\end{equation}

Con la intención de poder obtener una señal discreta fiel a la señal continua, se tomará para esta investigación, cumpliendo con el teorema de Nyquist, el intervalo indicado por la ecuación \ref{ec:teoremaNySreal}, considerando como  $f_{N}$ el ancho de banda de la señal, tal que  ${f}_{B} = \frac{\omega_{B}}{2\pi}$.
	\begin{equation}
	{2\omega_{B}}< {\omega_{s}}< 10 {\omega_{B} }
	\label{ec:teoremaNySreal}%
\end{equation}


Por lo tanto, la frecuencia de muestreo a utilizar \ref{ec:teoremaNySreal2}

%	\begin{equation}
%	{\omega_{s}} > 1883,8056 \quad rad/s
%	\label{ec:teoremaNySreal2}%
%\end{equation}

\begin{equation}
	470.9514 \quad rad/s < {\omega_{s}}< 2354,757 \quad rad/s
	\label{ec:teoremaNySreal2}%
\end{equation}

y el tiempo de muestreo $T_{s} = \frac{2\pi}{\omega_{s}}$ sería

%	\begin{equation}
% {T_{s}}>  	3,3353ms
%	\label{ec:tiempodemuestreo}%
%\end{equation}

	\begin{equation}
	2,668ms < {T_{s}}<  13,3414ms
	\label{ec:tiempodemuestreo}%
\end{equation}

El criterio escogido para el desarrollo de este proyecto es de $T_{s}= 3ms$.

\subsection{Discretización}\label{sec:discretizacion}

Para la discretización de un modelo continuo se contempla la posibilidad de trabajar con Euler en atraso, Euler en adelanto ó Tustin tambien conocido como Trapezoidal.

A continuación conoceremos mejor las ventajas y desventajas de cada método y cual fue escogido para el desarrollo de este trabajo.

\subsubsection{Euler en atraso:}

El método Euler se basa en aproximar a través de rectángulos de ancho $T_{s}$ el gráfico de una señal contínua. Se define el método en atraso tomando como referencia el instante $f((k+ 1)T_{s})$. Esto se aprecia en el gráfico de la figura  \ref{fig:Euleratraso}.


\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{img/euleratraso.jpg}
	\caption{Euler en atraso.}
	\label{fig:Euleratraso}
\end{figure}
Siendo $k$ el número de muestras.

La forma de este mapeo responde a la ecuación \ref{ec:Euleratraso}

\begin{equation}
	s=\dfrac{z-1}{T_{s}z}
	\label{ec:Euleratraso}%
\end{equation}  

Siendo $s$ la variable compleja asociada al dominio continuo según la transformada de Laplace y $z$ la variable compleja asociada al dominio discreto según la transformada Z.

Una de las ventajas de este método es que cuando se discretiza un sistema en tiempo continuo estable, se obtiene un resultado de tiempo discreto estable.

\subsubsection{Euler Adelanto:}

Como en la sección de Euler en atraso, este método tambien se basa en aproximar con rectángulos pero el instante de referencia que se toma es $f(kT_{s})$. El gráfico que representa este metodo viene dado por la figura \ref{fig:Euleradelanto}.


\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{img/euleradelanto.jpg}
	\caption{Euler en adelanto.}
	\label{fig:Euleradelanto}
\end{figure}

La forma del mapeo viene dada por la ecuación \ref{ec:Euleradelanto}

\begin{equation}
	s=\dfrac{z-1}{T_{s}}
	\label{ec:Euleradelanto}%
\end{equation}  

Euler en adelanto presenta una ventaja si el tiempo de muestreo es reducido, donde el límite de Nyquist es mayor comparado con el ancho de banda del controlador. En caso de un tiempo de muestreo mayor esté método puede generar inestabilidad.

\subsubsection{Trapezoidal, Bilineal ó Tustin:}

Este método es utilizado para aproximar a través de trapezoides el gráfico de una señal continua. Se aprecia en el gráfico de la figura \ref{fig:trapezoidal}.


\begin{figure}[H]
	\centering
	\includegraphics[scale=0.8]{img/trapezoidal.jpg}
	\caption{Trapezoidal, Bilineal ó Tustin.}
	\label{fig:trapezoidal}
\end{figure}

La ecuación que representa el mapeo es \ref{ec:trapezoidal}

\begin{equation}
	s=\dfrac{2(z-1)}{T_{s}(z+1)}
	\label{ec:trapezoidal}%
\end{equation}

Las ventajas de utilizar este método son las mismas del Euler en atraso pero de todas las fórmulas de integración disponibles, la trapezoidal tiene la coincidencia más cercana entre el dominio de frecuencia de un sistema discreto y el correspondiente sistema continuo.




\subsection{Modelo discreto}

\subsubsection{Comparación entre métodos de discretización:}

En la sección \ref{sec:discretizacion} se presentan tres opciones para la discretización del modelo continuo, los cuales fueron Euler en atraso, Euler en adelanto y Tustin, tomando en cuenta las ecuaciones de los mapeos que representan y sus principales carácteristicas.

En la tabla \ref{tab:Modelosdiscritizacion} se realiza una comparación entre los tres métodos con la finalidad de escoger el más adecuado para el desarrollo de este trabajo.

% Table generated by Excel2LaTeX from sheet 'Hoja1'
\begin{table}[H]
	\centering
	\caption{Comparación entre Los métodos de discretización.}
	\begin{tabular}{|p{8em}|p{8em}|p{5em}|p{8em}|}
		\hline
		\multicolumn{1}{|l|}{} & \textbf{¿Mantiene estabilidad al discretizar un sistema estable?} & \textbf{¿Necesita un tiempo de muestreo pequeño?} & \textbf{¿Coincidencia entre el gráfico de la señal en el dominio discreto y el sistema continuo?} \\
		\hline
		\textbf{Euler atraso} & Sí    & No    & Baja \\
		\hline
		\textbf{Euler adelanto} & No    & Sí    & Baja \\
		\hline
		\textbf{Tustin} & Sí    & No    & Alta \\
		\hline
	\end{tabular}%
	\label{tab:Modelosdiscritizacion}%
\end{table}%


En la tabla se muestra que el método de Tustin es el adecuado para la discretización del modelo continuo ya que es el que coincide mejor con las caráteristicas comparadas.

\subsubsection{Discretización del sistema de segundo orden:}

Partiendo de la ecuación \ref{ec:espaciodeestado2q}, se tiene:
\begin{center}
	\begin{equation}
		\left\{\begin{matrix}
			\begin{bmatrix}
				& \dot{\omega _{m}(t)}\\ 
				& \dot{I_{a}(t)}
			\end{bmatrix}
			=
			\begin{bmatrix}
				& 0 & 2340 & \\ 
				&  -61,22& -1,293\cdot10^{4} & \\ 
			\end{bmatrix}
			\begin{bmatrix}
				& \omega_{m}(t) \\ 
				& I_{a}(t)
			\end{bmatrix}
			+
			\begin{bmatrix}
				& 0 \\ 
				& 1220
			\end{bmatrix}
			e_{a}(t)\\ 
			Y=\begin{bmatrix}
				1 & 0
			\end{bmatrix}
			\begin{bmatrix}
				\omega_{m}(t)\\ 
				I_{a}(t)
			\end{bmatrix}
		\end{matrix}\right.
		%	\label{ec:espaciodeestado2q}%	
	\end{equation}
\end{center}

Luego, se realiza la discretización a través del programa \ref{code2} en MATLAB.

\begin{lstlisting}[caption = {Sistema de espacio de estados en Matlab},captionpos=b,label={code2},language=Matlab]
	% Constantes
	km=0.0502;
	jm=21.456E-6;
	rm=10.6;
	lm=0.82E-3;
	Tm=3E-3
	
	% Espacio de estados
	A=[0 km/jm; -km/lm -rm/lm];
	B=[0;1/lm];
	C=[1 0];
	D=0;
	
	%Discretizacion
	[Ad,Bd,Cd,Dd]=c2dm(A,B,C,D,Tm, 'tustin' );
	[num,den]=ss2tf(Ad,Bd,Cd,Dd,1);
	Gdiscreta = tf(num,den,Tm);
\end{lstlisting}

La ecuación que se obtiene es:

\begin{equation}
	G(z)=\dfrac{0.3099(z)^2 + 0.6199z + 0.3099}{(z)^2 - 0.06544 z - 0.8723}
	\label{ec:discretizacion}%
\end{equation}

En términos de $z^-1$

\begin{equation}
G(z)=\dfrac{0.3099 + 0.6199z^-1 + 0.3099 z^-2}{1 - 0.06544 z^-1 - 0.8723z^-2}
	\label{ec:discretizacionz}%
\end{equation}

\section{Diseño de la interfaz de emulación de la planta de segundo orden}\label{sec:diint}

De acuerdo con la tabla \ref{tab:requisitos}, la plataforma tecnológica seleccionada para el diseño esta basada en el lenguaje de programación Python. Para esto se contempla realizar una interfaz gráfica que contenga comunicación serial, cálculos matemáticos, modificación de texto y posibilidad de realizar gráficos, para lo cual se utiliza varios paquetes integrados como lo es TKinter.

\subsection{Interfaz gráfica}
Para el diseño de la interfaz gráfica se consideró el paquete Tkinter, que es parte de la herramientas que se pueden utilizar en Python de forma nativa. Esta herramienta se configuró para que se permita cambiar los parámetros que componen a la ecuación del sistema de segundo orden, además de contener las siguientes acciones:


\begin{description}
	\item[a.] \textbf{Selección del puerto para la comunicación serial con el controlador.}
	
	Se utilizó la librería pySerial \cite{serial} la cual es una extensión compatible con  Windows 32 bits, OSX, Linux, BSD, Jython y IronPython, permitiendo configurar el puerto a través de las propiedades de Python. 
	
	Se conecto el puerto serial a 921600 baudios con un timeout de 10 seg. Se habilitó una entrada de texto en la interfaz para poder insertar el puerto  por el cual se va a realizar la comunicación serial con el controlador real y un botón para conectarlo donde saldrá una notificación si la comunicación fue correcta o ocurrió un error en el proceso.
	
	El proceso de comunicación entre la interfaz de emulación y el controlador transcurre de la siguiente manera
	
	%Se utiliza los siguientes pasos para abrir la comunicación con el controlador:
	\begin{itemize}
		%\item Se presiona un botón que envía por el puerto serial un $"$o$"$ el cual significa que se abrió la comunicación, enviando un cero.
		\item La interfaz cuenta con un botón de inicio de comunicación. Este provoca que se envíe el carácter $"o"$ que se interpreta como un comando de inicio en el dispositivo controlador. De ser aceptada la respuesta, el microcontrolador devuelve cero como valor de la respuesta.
		\item El controlador calculará los valores respectivos según los reciba desde el sistema, para luego devolver los valores de la señal de control al sistema permitiendo repetir el ciclo. 
		
		%\item El controlador comienza a calcular los valores y se los envía al sistema.
		\item La comunicación termina cuando el ciclo el cual tiene una cantidad definida de valores acaba y cuando se haga uso del comando $"$c$"$.
		
		\begin{table}[htbp]
			\centering
			\caption{Lista de comandos enviados al controlador.}
			\begin{tabular}{|l|l|}
				\hline
				\multicolumn{2}{|c|}{Lista de comandos enviados al controlador} \\
				\hline
				carácter & función \\
				\hline
				$"$c$"$   & fin de comunicación \\
				\hline
				$"$o$"$   & inicio de comunicación \\
				\hline
			\end{tabular}%
			\label{tab:comandos}%
		\end{table}%
	\end{itemize}

En el diagrama \ref{fig:Pserial} se muestra como se ejecuta la comunicación inicial del puerto serial:

\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{img/pserial}
	\caption{Conexión puerto serial. }
	\label{fig:Pserial}
\end{figure}
	
	\item[b.] \textbf{Configuración de los parámetros del sistema de segundo orden.}
	
Para esto se preestablecieron los datos para el funcionamiento del motor Quanser. De igual forma, la interfaz permite modificar los parámetros para que se pueda emular cualquier sistema de segundo orden que cumpla con la ecuación dada en \ref{ec:plantasegundo}.

\begin{center}
	\begin{equation}
		\frac{Y(s)}{U(s)}= \frac{b_{0}}{a_{2}s^{2}+a_{1}s+a_{0}} 
		\label{ec:plantasegundo}%
	\end{equation}
\end{center}	

El código principal define arreglos para la entrada, salida y tiempo. Al obtener los parámetros de la interfaz, se definen los coeficientes $b_{0}$, $a_{2}$, $a_{1}$  y $a_{0}$ que son necesarios para estimar el resultado de la ecuación en diferencias del sistema discretizado. En el diagrama \ref{fig:confip} se muestra la configuración del sistema.


\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{img/confip}
	\caption{Configuración del sistema. }
	\label{fig:confip}
\end{figure}

Para obtener el sistema discretizado se debe tomar en cuenta las condiciones que se utilizaron para las derivadas. Para la derivada de segundo orden se utiliza:
\begin{center}
	\begin{equation}\label{ec:segundoor}
		s^{2}f=\frac{{f}'(t)-{f}'(t-1)}{t}=\frac{ \frac{f(t)-f(t-1)}{t} -  \frac{f(t-1)-f(t-2)}{t}}{t}
	\end{equation}
\end{center}

teniendo como resultado final

\begin{center}
	\begin{equation}\label{ec:segundoordfinal}
		s^{2}f=\frac{{f}(t)-2{f}(t-1)+f(t-2)}{t^{2}}
	\end{equation}
\end{center}


Una vez obtenida la derivada de segundo orden, se despeja de la ecuación \ref{ec:plantasegundo} obteniendo:

\begin{center}
	\begin{equation}\label{ec:plantaseg}
	Ub_{0}=a_{2}s^{2}y+a_{1}sy+a_{0}y
	\end{equation}
\end{center}

Tomando $y(t-1)=y_{1}$, $y(t-2)=y_{2}$ y $t_{m}=$ tiempo de muestreo, se obtiene \ref{ec:plantaseg1}:

\begin{center}
	\begin{equation}\label{ec:plantaseg1}
		Ub_{0}=a_{2}\left ( \frac{y-2y_{1}+y_{2}}{t_{m}^{2}} \right )+a_{1}s\left ( \frac{y-y_{1}}{t_{m}} \right )+a_{0}y
	\end{equation}
\end{center}

\begin{center}
	\begin{equation}\label{ec:yinicial}
		y_{inicial}=\frac{(Ub_{0}t_{m}^{2}+(2a_{2}+a_{1}t_{m})y_{1}-a_{2}y_{2}-a_{2}y_{2})}{a_{2}+a_{1}t_{m}+a_{0}t_{m}^{2}}
	\end{equation}
\end{center}
	
	
	\item[c.]\textbf{ Configuración de los parámetros del controlador.}
	
	La interfaz cuenta con unas entradas que permiten configurar los parámetros  del controlador PID que se desea utilizar. 
	
	\item[d.] \textbf{Graficar la respuesta controlada y sin controlar.}
	
	Una vez se termina la comunicación, se puede presionar el botón para obtener la respuesta controlada y la respuesta original del sistema. En el diagrama \ref{fig:Graficos} se muestra la configuración de los gráficos:
	
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\linewidth]{img/Graficos}
	\caption{Configuración de gráficos }
	\label{fig:Graficos}
\end{figure}
	 
	 
	\item[e.] \textbf{Obtener los datos de tiempo de alza, tiempo de establecimiento y sobrepico del sistema controlado.}
	
	Por último, después de graficar se mostrará los datos de sobrepico porcentual, tiempo de establecimiento y tiempo de alza tomados de los valores que proporciona la gráfica.
	
	\item [d.]\textbf{ Posibles errores del sistema.}
	En el inicio del código se definen las librerías que se utilizaran para el desarrollo del sistema de segundo orden, se decidirá el puerto serial de comunicación y la velocidad que tendrá el mismo. Las ventanas de error son las siguientes:
	\begin{itemize}
		\item Error 01: Error si el campo de puerto esta vacío.
		\item Error 02: Error si no hay conexión de puerto.
		\item Error 03: Error si no se ha configurado  la planta.
		\item Error 04: Error con la conexión.
	\end{itemize} 
\end{description}

%Se cuenta con un botón para iniciar la comunicación con el controlador y dos botones uno para graficar la respuesta controlada y otro para ver la respuesta original.

%Al abrir la comunicación con el controlador se envía por el puerto serial un $"$o$"$ de y se comienza la comunicación enviando un cero y luego calculando los valores del sistema dependiendo de los valores que envié el controlador y la comunicación acaba cuando se termina el ciclo de una cantidad definida de valores antes de  salir.

%Una vez terminada la comunicación se calcula los valores de sobrepico, tiempo de alza y tiempo de establecimiento, tomados de los valores que proporciona la gráfica.


En el siguiente diagrama se muestra la manera de en que se ejecuta la comunicación:

	\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\linewidth]{img/confic}
	\caption{Configuración de comunicación }
	\label{fig:confic}
\end{figure}



La trama de comunicación entre el sistema y el controlador consta de un campo variable de máximo 6 caracteres que representan el valor numérico decimal que se manejará en las ecuaciones y un caracter de salto de línea para culminar el mensaje. El valor enviado o recibido debe ser procesado según corresponda. Es decir, para enviar al controlador el valor a = 123.56, debe multiplicarse por cien, quedando a= 12356

Una vez hecho esto, se transforma de valor numérico 123456 a una cadena de caracteres $"$12356$"$ a la cuál se adiciona un salto de línea para concluir el mensaje, quedando de la siguiente manera:


Mensaje a enviar = $"$123456 0x0A$"$
%La trama de la comunicación entre el sistema y el controlador consta de un protocolo de comunicación entre el controlador y el emulador donde una vez validados los campos en el programa, se convierten en un tipo de dato float y comienza la comunicación.  


% donde el usuario incorpora los coeficientes que componen el sistema de segundo orden mostrado en la ecuación \ref{ec:plantasegundo} ; una vez validados los campos en el programa, se convierten en un tipo de dato float y comienza la comunicación.

\begin{table}[htbp]
	\centering
	\caption{Trama de comunicación}
	\begin{tabular}{|l|l|c|}
		\hline
		\multicolumn{3}{|c|}{Estructura de trama de comunicación} \\
		\hline
		& Descripcion & \multicolumn{1}{l|}{Tamaño} \\
		\hline
		Inicio & valor decimal x 100 & \multicolumn{1}{l|}{Maximo 5} \\
		\hline
		Final & $"$0x0A$"$ o $"$Salto de linea$"$ & 1 \\
		\hline
	\end{tabular}%
	\label{tab:trama}%
\end{table}%


Los condiciones iniciales se toman en reposo al inicio, por este motivo son iguales a cero. Luego se comienza un ciclo que comenzará a contar y a calcular los valores del arreglo con respecto los coeficientes de la ecuación y con el tiempo de muestreo.





Con el valor obtenido en \ref{ec:yinicial} se va creando un arreglo con el método $"$np.append$"$ que es parte de la librería Numpy. Este método permite agregar la $y_{inicial}$ al final del arreglo e ir obteniendo todos los valores. El código continua cambiando en cada ciclo los valores de $y_2=y_{1}$  y $y_1=y_{inicial}$.

El programa evalúa mediante un comando condicional si el puerto esta funcionando y si los parámetros están configurados correctamente. En caso de ser cierto se limpia el buffer de comunicación, los arreglos de tiempo y los valores que tomara la gráfica para evitar datos erroneos.


Se manda por el puerto serial una $"$o$"$ y comienza el timeout que es el tiempo que dice cuánto dura la comunicación, se lee el puerto serial con el método readline y se guarda en la variable  $"$lect $"$.


Una vez recibido un nuevo valor a través del puerto serial, se evalúa la longitud del mismo con el método $"$len$"$ , ya que pueden existir valores nulos. De ser así, en el arreglo U se inserta un 0 con ayuda del método np.insert. De no ser nulo el valor recibido, se calcula la respuesta a través de \ref{ec:yinicial}. 

%Luego el método $"$len$"$ que devuelve la longitud de una lista, se pone con un comando condicional de que si es estrictamente igual a cero, se con el método np.insert se crea un arreglo de ceros si esto no se cumple se calcula en valor dado en \ref{ec:yinicial} .


%\begin{center}
%	\begin{equation}\label{ec:ecupro}
%		y=\frac{float(\frac{int(lect)}{100})b_{0}t_{m}^{2}+(2a_{2}+a_{1}t_{m})y_{1}-a_{2}y_{2}-a_{2}y_{2}}{a_{2}+a_{1}t_{m}+a_{0}t_{m}^{2}}
%	\end{equation}
%\end{center}

Dividiendo el coeficiente que se obtiene del controlador y convirtiendolo en un valor de tipo float, luego se multiplica la respuesta por 100 y se convierte en un valor de tipo int enviándolo de respuesta al controlador mas un salto de linea. Por último, se aumenta el contador del tiempo.

Para salir del ciclo cada vez que se calcula un valor de la respuesta se aumenta un contador y se compara cuándo se cumple el timeout establecido, si el contador es mayor que el timeout se envía a la interfaz el mensaje de $"$Terminado$"$, y se manda el comando de finalización de la comunicación, según como se muestra en la tabla  \ref{tab:comandos}.

 




