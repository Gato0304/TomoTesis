De acuerdo con la tabla \ref{tab:requisitos} ubicada en el capítulo \ref{CAP:tecni}, donde se realiza una comparación entre las herramientas para la realizar la emulación del sistema, la plataforma tecnológica seleccionada para el diseño está basada en el lenguaje de programación Python. Para esto se contempló realizar una interfaz gráfica que contenga comunicación serial, cálculos matemáticos, modificación de texto y posibilidad de mostrar el gráfico de las señales, para lo cual se utilizaron varios paquetes integrados como lo es TKinter, Matplotlib y Numpy.
...
En la interfaz se pueden cambiar las configuraciones predeterminadas del sistema de segundo orden, puerto serial y los parámetros del controlador.

\subsection{Configuración del puerto}

Previo a todas las tareas que se pueden llevar a cabo en el sistema, fue necesario como primer paso configurar el del puerto serial que se utilizó como medio de comunicación con el controlador. 

Por lo tanto, se considera el uso de la librería pySerial \cite{serial} que es una extensión compatible con  Windows 32 bits, OSX, Linux, BSD, Jython y IronPython, permitiendo configurar el puerto a través de las propiedades de Python. 

Para la comunicación del puerto se establece una tasa de 115200 baudios con un timeout de 10 segundos.

Luego de configurar el puerto serial correctamente, se habilitan las opciones de configuración de la planta, configuración del controlador y los gráficos. En el diagrama de la figura \ref{fig:Pserial} se muestra el proceso de configuración del puerto serial.

	\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\linewidth]{img/pserial}
	\caption{Configuración de la conexión del puerto serial. }
	\label{fig:Pserial}
\end{figure}
...
Se desea emular cualquier sistema de segundo orden que cumpla con la ecuación general dada a continuación:
...
Siendo $Y_{z}$ la respuesta a la salida del sistema y $U_{z}$ la señal de control:
\begin{equation}
...
La ecuación \ref{ec:salidaDiscretaCodigo} es una representación discreta del sistema dado por la ecuación \ref{ec:discretizacionSO}, la cual será utilizada para el desarrollo del algoritmo en Python. 
...
En el diagrama de la figura \ref{fig:confip} se muestra como configurar el sistema de segundo orden en la interfaz. 
	...
	\caption{Proceso de configuración de los parámetros de la planta de segundo orden. }
	\label{fig:confip}
\end{figure}
...
La trama de comunicación entre el sistema y el controlador consta de un campo de caracteres variables, que representan el valor numérico decimal que se manejará en las ecuaciones y un caracter de salto de línea (ASCII hexadecimal: 0x0A) para culminar el mensaje. El valor enviado o recibido debe ser procesado según corresponda. Es decir, para enviar al controlador el valor $a = 123.56$, debe multiplicarse por $100000000$, quedando $a= 12356000000$

Una vez hecho esto, se transforma de valor numérico 12356000000 a una cadena de caracteres ``12356000000'' a la cual se adiciona un salto de línea para concluir el mensaje, quedando de la siguiente manera:
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|l|c|}
		...
		\hline
		& Descripción & \multicolumn{1}{l|}{Tamaño} \\
		\hline
		Inicio & valor decimal x 100000000 & \multicolumn{1}{l|}{Variable} \\
		\hline
		Final & ``0x0A$"$ o ``Salto de línea$"$ & 1 \\
		\hline
	\end{tabular}%
	\label{tab:trama}%
\end{table}%
...
El proceso de comunicación de la planta con el controlador se detalla en el diagrama de la figura \ref{fig:confic}:

\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\linewidth]{img/confic}
	\caption{Proceso de comunicación entre la planta y el controlador  }
	\label{fig:confic}
\end{figure}

\subsection{Validaciones previas}

El programa evalúa mediante un comando condicional si el puerto está funcionando y si los parámetros están configurados correctamente. En caso de ser cierto, se limpia el buffer de comunicación, los arreglos de tiempo y los valores que tomará el gráfico de resultados para evitar datos erróneos.
...
Una vez recibido un nuevo valor desde el puerto serial, se evalúa la longitud del mensaje, ya que pueden existir valores vacíos. Si la longitud es la correcta, se procederá a procesar la respuesta dada en la ecuación \ref{ec:salidaDiscretaCodigo}. En el caso contrario, el valor se ignora.
