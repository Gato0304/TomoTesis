En este capítulo se explican los fundamentos necesarios para el modelado de la planta de segundo orden y el motor escogido para tener un referente en el sistema. A demás, se realiza una comparación entre el circuito Sallen-Key y el sistema emulado.  
...
Entonces en las unidades del sistema SI, los valores de $k_{b}$ y $k_{i}$ son iguales.
 \section{DCMCT}
...
Con la tabla \ref{tab:addlabel} se estudia el error porcentual asociado a estos modelos con la ecuación  \ref{ec:errorporcentual}: 
\begin{center}
	\begin{equation}
...
De alli se obtiene que para el tiempo de alza y para el tiempo de establecimiento los errores serian:
...
De acuerdo con la tabla \ref{tab:requisitos}, la plataforma tecnológica seleccionada para el diseño esta basada en el lenguaje de programación Python. Para esto se contempla realizar una interfaz gráfica que contenga comunicación serial, cálculos matemáticos, modificación de texto y posibilidad de realizar gráficos, para lo cual se utiliza varios paquetes integrados como TKinter.
...
\begin{description}
	...
	Se conecto el puerto serial a 921600 baudios con un timeout de 10 seg. Se habilitó una entrada para poder insertar el puerto  por el cual se va a realizar la comunicación serial con el controlador real y un botón para conectarlo donde saldrá un mensaje de si la comunicación fue correcta o ocurrió un error en el proceso.
	...
	\begin{itemize}
		...
		\item La interfaz cuenta con un botón de inicio de comunicación. Este provoca que se envíe el carácter $"o"$ que se interpreta como un comando de inicio en el dispositivo controlador. De ser aceptada la respuesta, el microcontrolador debe devolver un valor de cero.
		...
		\item La comunicación termina cuando acaba el ciclo el cual tiene una cantidad definida de valores.
	\end{itemize}

En el siguiente diagrama se muestra como se ejecuta la comunicación del puerto serial:
...
Para esto se preestablecieron los datos para el funcionamiento del motor Quanser,  se permiten cambiar los parametros para que se pueda emular cualquier sistema de segundo orden que cumpla con la ecuación dada en \ref{ec:plantasegundo}.
...
Luego se definen los arreglos vacíos de entrada, salida y tiempo, con los campos de entrada respectivos y se obtienen los valores del sistema de segundo orden; las variables de los pasos siguientes se definen con valor de cero. En el diagrama \ref{fig:confip} se muestra la configuración del sistema.
...
La trama de la comunicación entre el sistema y el controlador consta de los campos de entrada donde el usuario incorpora los coeficientes que componen el sistema de segundo orden mostrado en la ecuación \ref{ec:plantasegundo} ; una vez validados los campos en el programa se convierten en un tipo de dato float y comienza la comunicación.


Los condiciones iniciales se toman en reposo al inicio, por este motivo son iguales a cero. Luego se comienza un ciclo que comenzará a contar y a calcular los valores del arregle con respecto los coeficientes de la ecuación y con el tiempo de muestreo.


\begin{center}
	\begin{equation}\label{yinicial}
		y_{inicial}=\frac{(b_{0}t_{m}^{2}+(2a_{2}+a_{1}t_{m})y_{1}-a_{2}y_{2}-a_{2}y_{2})}{a_{2}+a_{1}t_{m}+a_{0}t_{m}^{2}}
	\end{equation}
\end{center}


Con el valor obtenido en \ref{yinicial} se va creando un arreglo con el método $"$np.append$"$ que es parte de la librería Numpy. Este método permite agregar la $y_{inicial}$ al final del arreglo e ir obteniendo todos los valores.El código continua cambiando en cada ciclo los valores de $y_2=y_{1}$  y $y_1=y_{inicial}$.

El programa evalúa mediante un comando condicional si el puerto esta funcionando y si los parámetros están configurados correctamente. En caso de ser cierto se limpia el buffer de comunicación para evitar datos erroneos, luego se declaran los arreglos de tiempo y de los valores que tomara la gráfica vacios.


Se mandan por el puerto serial una $"$o$"$ y comienza el timeout que es el tiempo que dice cuanto dura la comunicación, se lee el puerto serial con el comando readline y se guarda en la variable  $"$lect $"$.


Luego el método $"$len$"$ que devuelve la longitud de una lista, cadena, se pone con un comando condicional de que si es estrictamente igual a cero, se con el método np.insert se crea un arreglo de ceros si esto no se cumple se calcula en valor dado en \ref{ec:ecupro} .
...
Dividiendo el coeficiente que se obtiene del controlador y convirtiendolo en un valor de tipo float, luego se multiplica $"$y$"$ por 100 y se convierte en un valor de tipo int enviándolo de respuesta al controlador mas un salto de linea. Por último, se aumenta el contador del tiempo.

Para salir del ciclo cada vez que se calcula un valor de $"$y$"$ se aumenta un contador y se compara cuando el timeout, si el contador es mayor que el timeout se envía a la interfaz el mensaje de $"$Terminado$"$, y por el puerto serial se manda una $"$c$"$ para culminar la comunicación. 
